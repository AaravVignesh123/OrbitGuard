...
Four tabs (Overview / Live Report / How it works / Roadmap) with a premium,
Cluely-inspired feel implemented in real code (no external deps): an animated
aurora background, a cursor spotlight, scroll-triggered reveals, count-up
numbers, parallax floating UI cards, a live conjunction ticker, magnetic
buttons, and shimmer — all gated behind ``prefers-reduced-motion``. Fused with
OrbitGuard's mission-control data aesthetic (mono technical labels, hairline
rules) so it reads as a product, not a landing-page template.

Everything (including Plotly) is inlined — open the file directly, or host
``docs/index.html`` on GitHub Pages. Built with token replacement (not
str.format) so the CSS braces stay readable.
"""
...
def _build_figure(geometry: List[dict]) -> go.Figure:
    ...
    fig.add_trace(go.Surface(
        ...
        hoverinfo="skip", name="Earth", lighting=dict(ambient=0.7, diffuse=0.55)))
    ...
    for gi, g in enumerate(geometry):
        ...
        fig.add_trace(go.Scatter3d(x=arc_a[:, 0], y=arc_a[:, 1], z=arc_a[:, 2], mode="lines",
            line=dict(color="#5cc8ff", width=4), name=g["object_a"], visible=vis, hoverinfo="name"))
        fig.add_trace(go.Scatter3d(x=arc_b[:, 0], y=arc_b[:, 1], z=arc_b[:, 2], mode="lines",
            line=dict(color="#ff4d8d", width=4), name=g["object_b"], visible=vis, hoverinfo="name"))
        fig.add_trace(go.Scatter3d(x=[pa[0]], y=[pa[1]], z=[pa[2]], mode="markers",
            marker=dict(size=5, color="#5cc8ff"), name=g["object_a"] + " @ TCA", visible=vis, hoverinfo="name"))
        fig.add_trace(go.Scatter3d(x=[pb[0]], y=[pb[1]], z=[pb[2]], mode="markers",
            marker=dict(size=5, color="#ff4d8d"), name=g["object_b"] + " @ TCA", visible=vis, hoverinfo="name"))
        fig.add_trace(go.Scatter3d(x=[pa[0], pb[0]], y=[pa[1], pb[1]], z=[pa[2], pb[2]],
            ...
            textfont=dict(color="#f5a524", size=13), name="miss distance", visible=vis, hoverinfo="text"))
    ...
    fig.update_layout(
        ...
        updatemenus=[dict(buttons=buttons, direction="down", showactive=True,
            ...
            bgcolor="#0e1626", font=dict(color="#e6ecf5", size=12), bordercolor="#2a3a57",
        )] if geometry else [],
        scene=dict(
  ...
  <div class="hero-grid">
    <div class="hero-main">
      ...
      <div class="cta" data-reveal>
        <a class="btn primary magnet" data-goto="report"><span>Open the live report</span><span class="arr">→</span></a>
...
<section class="ctaband" data-reveal>
  <div class="ctaband-in">
    <div><div class="kicker"><span class="sec-no">→</span>SEE IT LIVE</div>
      <h2>The full catalogue, screened and ranked.</h2></div>
    <a class="btn primary magnet" data-goto="report"><span>Open the live report</span><span class="arr">→</span></a>
  </div>
</section>
"""
...
def _roadmap() -> str:
    phases = [
        ("PHASE 0–2.5", "v1 — Conjunction screener", "done",
         "Live catalogue → propagate → screen → refine → risk-rank → report + dashboard. Validated: KD-tree "
...
/* animated aurora + starfield background */
...
.bg::before, .bg::after { content:""; position:absolute; width:70vw; height:70vw; border-radius:50%;
  filter:blur(120px); opacity:.5; }
.bg::before { background:radial-gradient(circle, rgba(46,90,180,.55), transparent 60%); top:-24vw; right:-14vw;
  animation:drift1 26s ease-in-out infinite; }
.bg::after { background:radial-gradient(circle, rgba(139,123,255,.34), transparent 60%); bottom:-28vw; left:-16vw;
...
#spot { position:fixed; width:520px; height:520px; border-radius:50%; z-index:-2; pointer-events:none;
  background:radial-gradient(circle, rgba(92,200,255,.10), transparent 65%); transform:translate(-50%,-50%);
  left:50%; top:20%; transition:opacity .4s; opacity:0; }
...
.nav { position:sticky; top:0; z-index:50; backdrop-filter:blur(14px);
  background:rgba(4,6,13,.7); border-bottom:1px solid var(--hair); }
...
.brand .g { color:var(--dim); font-weight:600; } .brand .b { color:var(--ink); }
...
.tab.active::after { content:""; position:absolute; left:14px; right:14px; bottom:-1px; height:2px;
  background:var(--accent); border-radius:2px; box-shadow:0 0 10px rgba(92,200,255,.7); }
...
.hl::after { content:""; position:absolute; left:2px; right:6px; bottom:7px; height:11px; z-index:-1; border-radius:3px;
  background:linear-gradient(90deg, rgba(245,165,36,0), rgba(245,165,36,.5)); }
.cta { display:flex; gap:12px; flex-wrap:wrap; margin-top:28px; }
.btn { display:inline-flex; align-items:center; gap:9px; padding:13px 22px; border-radius:12px; font-weight:600;
  font-size:14.5px; border:1px solid transparent; position:relative; overflow:hidden; will-change:transform; }
...
.btn.ghost { border-color:var(--hair2); color:var(--ink); background:rgba(255,255,255,.02); }
.btn.ghost:hover { background:rgba(255,255,255,.06); }
.hero-cmd { font-family:var(--mono); font-size:12.5px; color:#9fb6d0; margin-top:22px; opacity:.85; }
.hero-cmd .prompt { color:var(--accent); margin-right:8px; }

/* hero floating cards */
.hero-stage { position:relative; height:460px; }
.fc-wrap { position:absolute; will-change:transform; }
.float-card { border:1px solid var(--hair2); border-radius:16px; background:linear-gradient(180deg, rgba(18,26,44,.86), rgba(9,13,22,.86));
  box-shadow:0 40px 80px -30px rgba(0,0,0,.85), inset 0 1px 0 rgba(255,255,255,.05); backdrop-filter:blur(8px); }
.fc-anim { animation:floaty 7s ease-in-out infinite; }
@keyframes floaty { 0%,100%{transform:translateY(0);} 50%{transform:translateY(-12px);} }
.fc-main { right:0; top:30px; width:290px; z-index:2; }
.fc-main .float-card { padding:16px 16px 12px; }
.fc-alert { left:-14px; top:236px; width:262px; z-index:4; }
.fc-alert .fc-anim { animation-duration:6s; animation-delay:-1.5s; }
.fc-alert .float-card { padding:15px 16px; }
.fc-orbit { right:150px; top:-6px; width:132px; z-index:1; }
.fc-orbit .fc-anim { animation-duration:8s; animation-delay:-3s; }
.fc-orbit .float-card { padding:6px; }
.orb svg { width:100%; display:block; }
.orbit-a { transform-origin:60px 60px; animation:spin 18s linear infinite; }
.orbit-b { transform-origin:60px 60px; animation:spin 24s linear infinite reverse; }
@keyframes spin { to { transform:rotate(360deg); } }

.ro-head { display:flex; justify-content:space-between; font-family:var(--mono); font-size:10px; letter-spacing:.18em;
  color:var(--faint); padding-bottom:11px; margin-bottom:4px; border-bottom:1px solid var(--hair); }
.ro-live { color:var(--green); } .ro-row { display:flex; justify-content:space-between; align-items:baseline; padding:8px 0; border-bottom:1px solid var(--hair); }
.ro-row:last-of-type { border-bottom:0; }
.ro-lab { font-family:var(--mono); font-size:10px; letter-spacing:.12em; color:var(--faint); }
.ro-val { font-family:var(--mono); font-size:18px; color:var(--ink); font-weight:600; }
.ro-val i { font-style:normal; font-size:11px; color:var(--dim); margin-left:2px; }
...
.fc-grid span { display:block; font-family:var(--mono); font-size:9px; letter-spacing:.1em; color:var(--faint); }
...
.tk-mask { position:relative; overflow:hidden; flex:1;
  -webkit-mask-image:linear-gradient(90deg, transparent, #000 6%, #000 94%, transparent);
          mask-image:linear-gradient(90deg, transparent, #000 6%, #000 94%, transparent); }
.tk-track { display:flex; gap:26px; width:max-content; animation:scroll 48s linear infinite; }
...
.tk b { color:#cfe0f4; font-weight:600; } .tk i { color:var(--faint); font-style:normal; }
.tk u { color:var(--accent); text-decoration:none; } .tk s { color:var(--faint); text-decoration:none; }
...
.aud-tag { font-family:var(--mono); font-size:10px; letter-spacing:.2em; color:var(--accent); border:1px solid var(--hair2);
  border-radius:6px; padding:3px 8px; display:inline-block; margin-bottom:12px; }

/* stats (report) */
.stats { display:grid; grid-template-columns:repeat(auto-fit,minmax(160px,1fr)); gap:1px; background:var(--hair);
  border:1px solid var(--hair); border-radius:14px; overflow:hidden; }
.stat { background:var(--bg2); padding:20px; } .stat-val { font-family:var(--mono); font-size:26px; font-weight:600; color:var(--ink); }
...
.nl-col.now { border-color:rgba(52,211,153,.28); } .nl-col.later { border-color:rgba(92,200,255,.24); }
...
.nl-col.now .nl-tag { color:var(--green); } .nl-col.later .nl-tag { color:var(--accent); }
...
.nl-col li::before { content:""; position:absolute; left:2px; top:15px; width:6px; height:6px; border-radius:1px; background:var(--faint); }
.nl-col.now li::before { background:var(--green); } .nl-col.later li::before { background:var(--accent); }
...
/* CTA band */
.ctaband { margin-top:80px; border:1px solid var(--hair2); border-radius:20px; padding:36px 40px; position:relative; overflow:hidden;
  background:linear-gradient(120deg, rgba(46,90,180,.22), rgba(139,123,255,.16)); }
.ctaband-in { display:flex; align-items:center; justify-content:space-between; gap:24px; flex-wrap:wrap; }
.ctaband h2 { font-size:26px; margin:8px 0 0; }
...
.flow { display:flex; flex-wrap:wrap; align-items:center; gap:10px; font-family:var(--mono); font-size:12.5px; color:var(--dim); margin-bottom:26px; }
...
.step { display:flex; gap:20px; border:1px solid var(--hair); border-radius:12px; padding:18px 20px; background:var(--panel); transition:border-color .2s, transform .2s; }
.step:hover { border-color:var(--hair2); transform:translateX(4px); }
.step-num { font-family:var(--mono); font-size:15px; font-weight:600; color:var(--accent); min-width:26px; padding-top:1px; }
.step-body h3 { font-size:16px; margin:0 0 5px; } .step-body p { color:var(--dim); font-size:14px; margin:0; }
...
.phase-dot { width:13px; height:13px; border-radius:50%; margin-top:5px; flex:none; z-index:1; background:var(--bg2); border:1px solid var(--faint); }
...
.risk { display:inline-block; min-width:38px; text-align:center; padding:3px 9px; border-radius:7px; font-family:var(--mono);
  font-weight:600; font-size:12px; color:hsl(var(--rh),85%,72%); background:hsla(var(--rh),80%,45%,.14); border:1px solid hsla(var(--rh),80%,55%,.4); }
.note { color:var(--faint); font-size:12.5px; margin-top:12px; line-height:1.65; }

footer { margin-top:72px; color:var(--faint); font-size:12px; font-family:var(--mono); letter-spacing:.03em;
...
<div class="wrap">
  ...
  <div class="view" id="view-report">
    ...
    <section>
      ...
      <div class="panel" data-reveal>__FIGURE__</div>
      <div class="note" data-reveal>Blue and pink arcs are the two objects' orbit tracks in the ±12&nbsp;min around
      closest approach (Earth-centred inertial frame); the dotted amber line is the miss distance at the time of
      closest approach (TCA). Use the dropdown to switch events; drag to rotate, scroll to zoom.</div>
    </section>
    <section>
      <div class="sec-head" data-reveal style="border:0;padding:0;margin-bottom:14px">
...
(function(){
  ...
  function countUp(el){
    ...
    function step(ts){ if(!start) start = ts; var p = Math.min((ts-start)/dur, 1);
      var e = 1 - Math.pow(1-p, 3); el.textContent = fmt(target*e, dec); if(p<1) requestAnimationFrame(step); }
    requestAnimationFrame(step);
  }

  // reveal on scroll (+ fire count-ups when revealed)
  ...
  // when switching tabs, reveal + count that view's in-viewport elements
  function revealView(view){
    view.querySelectorAll('[data-reveal]').forEach(function(el){
      if(!el.classList.contains('in')){ el.classList.add('in'); el.querySelectorAll('.count').forEach(countUp); }
    });
  }
  ...
  function show(name){
    ...
    if(view){ revealView(view); }
    if(name==='report'){ var el=document.getElementById('__SCENE_ID__'); if(el&&window.Plotly){ try{Plotly.Plots.resize(el);}catch(e){} } }
    if(history && history.replaceState){ history.replaceState(null,'','#'+name); }
    window.scrollTo({top:0});
  }
  document.addEventListener('click', function(e){
    var t=e.target.closest('[data-view]'), g=e.target.closest('[data-goto]');
    if(t){ show(t.dataset.view); } else if(g){ show(g.dataset.goto); }
  });

  if(!reduce){
    // cursor spotlight + parallax floating cards
    var spot = document.getElementById('spot');
    var cards = [].slice.call(document.querySelectorAll('.parallax'));
    var mx=0, my=0, tx=0, ty=0, raf=false;
    window.addEventListener('mousemove', function(e){
      mx = e.clientX; my = e.clientY;
      spot.style.opacity = 1; spot.style.left = mx+'px'; spot.style.top = my+'px';
      var nx = (e.clientX/window.innerWidth - .5), ny = (e.clientY/window.innerHeight - .5);
      tx = nx; ty = ny;
      if(!raf){ raf=true; requestAnimationFrame(applyParallax); }
    });
    function applyParallax(){
      cards.forEach(function(c){ var d = +c.dataset.depth || 12;
        c.style.transform = 'translate3d('+(-tx*d)+'px,'+(-ty*d)+'px,0)'; });
      raf=false;
    }
    // magnetic buttons
